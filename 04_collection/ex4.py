...
# List Comprehension vx Generator 표현식
...
print(f"{hours.index(max(hours))}요일")                                                    # ✅ 토요일

hpur, day = max(zip(hours, days))
print(f"{day}요일")
...
print(f"max점수: {result[-1]}점, min 점수: {result[0]}점")                                                    # ✅ max 점수: 92점, min 점수: 76점
...
total = sum([price * num for name, price, num in stocks])     # 리스트를 만들고 구해서 메모리가 좀 더 많음
